...
namespace Gw2_GuildEmblem_Cdn.Core.Custom.Gw2SharpWebApi.Caching
{
    public class DelayedExpiryMemoryCacheMethod : MemoryCacheMethod
    {
        ...
        public override Task<CacheItem<T>> GetOrUpdateAsync<T>(string category, string id, Func<Task<(T, DateTimeOffset)>> updateFunc)
        {
            ...
            {
                var fItem = await TryGetAsync<T>(category, id).ConfigureAwait(false);
                if (fItem != null)
                    return fItem;
                ...
                await SetAsync(cache).ConfigureAwait(false);
                return cache;
            }
        }
            ...
            Func<IList<string>, Task<(IDictionary<string, T>, DateTimeOffset)>> updateFunc)
        {
            ...
            async Task<IList<CacheItem<T>>> ExecAsync()
            {
                ...
                var cache = await GetManyAsync<T>(category, idsList).ConfigureAwait(false);
                ...
                if (missing.Count > 0)
                {
                    ...
                    await SetManyAsync(newCacheItems).ConfigureAwait(false);
